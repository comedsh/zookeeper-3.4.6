...
public class WriteLockTest extends ClientBase {
	...
	protected WriteLock[] nodes; // 试图得到锁的节点...
	
	protected CountDownLatch latch = new CountDownLatch(1); // 因为锁的获取是异步的，所以 Latch 的目的是，阻塞当前进程，当 node 成功获得锁以后，解除阻塞。
	...
	class LockCallback implements LockListener {
		...
		public void lockAcquired() {
			...
			System.out.println("================== 某个 node 成功获得了'锁' ================ ");
		...
		public void lockReleased() {
			...
			System.out.println("================== 某个 node 成功释放了'锁' ================ ");
	...
	protected void runTest(int count) throws Exception {
		...
		/** latch.await 的主要目的是让当前的线程休眠，并且等待 latch count down 到 0 为止，然后取消阻塞 **/
		/** 那作者为什么补充说明要等待之前的 leaders 死亡？应该是为了保证该测试用例的可重复执行，确保上一次执行完成以后 leader 死亡，因为只有上一个 leader 死亡以后，新的 nodes 才有可能获得锁 **/
		...
		if (count > 1) {
			...
			if ( killLeader ) {
				...
				/** 将会回调 LockCallback#lockRelease() **/
				first.unlock();
				
				/** 当 first node 锁释放以后，second node 会立马获得该锁，调用 LockCallback#lockAcquired() 并执行 Latch CountDown，然后阻塞取消 **/
				latch.await(30, TimeUnit.SECONDS);
